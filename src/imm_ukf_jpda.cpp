...
const double distanceThres_ = 5.5;
const int lifeTimeThres_ = 2;
//bbox update params
...
void getOriginPoints(double timestamp, vector<vector<double>>& originPoints, double egoyaw, float vel){
    ...
    // egoYaw -= (M_PI/3 +0.07);
    if(egoPoints_.size()>0){
    egoPoints_[0][0] = x;
    egoPoints_[0][1] = y;
    egoPoints_[0][2] = egoYaw;
    originPoints = egoPoints_;
    // cout<<"Number od ego points before assert"<<egoPoints_.size()<<endl;
...
void measurementValidation(vector<vector<double>> trackPoints, UKF& target, bool secondInit, VectorXd maxDetZ, MatrixXd maxDetS,
   ...
   cout<<"starting measurement validation"<<endl;
    int count = 0;
...
void filterPDA(UKF& target, vector<VectorXd> measVec, vector<double>& lambdaVec){
    ...
    for(int i = 0; i < numMeas; i++){
        ...
        VectorXd diffRM   = measVec[i] - measVec[i];
    ...
    double lambdaCV, lambdaCTRV, lambdaRM=0.0;
    if(numMeas != 0){
...
void associateBB(int trackNum, vector<VectorXd> bboxVec, UKF& target){
    ...
    cout<<"Target's lifetime"<< target.lifetime_<<endl;
    if(target.lifetime_ >= lifeTimeThres_){
...
// -4.74403 6.51258
    ...
    if(!init_) {
        ...
        cout <<"Here I am GOD help me"<< trackPoints.size()<< endl;
    	for(int i = 0; i < trackPoints.size(); i++){
    ...
    cout<<"COUNTIT VARIABLE"<< "  "<< countIt<<endl;
    if(countIt == 590) {
        cout<<"WENT THORUGH ALL FRAMES SUCKER"<<endl;
        // inFile_.close();
        yawFile_.close();
    }
    ...
    cout<<" Starting the UKF process printing targets size----------"<<targets_.size()<<" "<<trackNumVec_.size()<<endl;
    // start UKF process
    for(int i = 0; i < targets_.size(); i++){
        ...
        cout<<"just before associate BBs"<< trackNumVec_[i]<<endl;
    ...
    cout<<"Size of tragets vector HERE is *************************"<< targets_.size()<<endl;
    cout<<"Size of vehicles tracking vector HERE is **************************"<<trackNumVec_.size()<<endl;
}
